...
for(h2 in c(0.3)){
  ...
  for (i in 1:100){
    ...
    main_cpu_time_no_encode <- system.time(glmmkin_no_encode <- david_glmmkin.ai(fit0 = glm(y~1), kins = list(GRM), k = 0, verbose = T, encode = F,tol=0.01))
    
    theta_param_no_encode <- rbind(theta_param_no_encode,
                                   data.frame(h2=h2,sigma_e=glmmkin_no_encode$theta[1],sigma_g=glmmkin_no_encode$theta[2])
    )
    
    time_run_no_encode <- rbind(time_run_no_encode,c(main_cpu_time_no_encode))
    ...
    run_encoded_lmm <- function(T){
      ...
      gcode.model <- gcode::gcode(data_list = list(X.s), config = config, join = join)
      sample_encode <- t(scale(t(gcode.model$main.parameters$alpha_sample[[1]])))
      
      glmmkin_yes_encode <- david_glmmkin.ai(fit0 = glm(y~1), kins = list(GRM), k = k, verbose = T, encode = T, sample_encode = sample_encode, tol = 0.01)
    ...
    theta_param_yes_encode <- rbind(theta_param_yes_encode,
                                    data.frame(h2=h2,sigma_e=glmmkin_yes_encode$theta[1],sigma_g=glmmkin_yes_encode$theta[2])
    )
    
    time_run_yes_encode <- rbind(time_run_yes_encode,c(main_cpu_time_yes_encode))
    
  }
  
}

save.image("~/Documents/main_files/AskExplain/lmform/data/workflow/lmm/lmm_fast_timing.RData")
